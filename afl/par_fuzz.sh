...
# Start main node
tmux new -d -s "afl01" -n $tmux_window "cargo afl fuzz -t 20000 -i $in_dir -o $sync_dir -M fuzzer01 target/debug/afl" 
echo "Spawned main instance afl01"

# Start secondary instances
for i in $(seq -f "%02.0f" 2 "$USED_CORES"); do
    tmux new -d -s "afl$i" -n $tmux_window cargo afl fuzz -t 20000 -i $in_dir -o $sync_dir -S "fuzzer$i" target/debug/afl
